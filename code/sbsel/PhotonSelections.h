...
Int_t PassPhotonPreselections(TreeReader &data, Int_t i) {
  ...
  int bin;
  if (phoSCEta[i] < 1.0) bin = 0;
  else if (phoSCEta[i] >= 1.0 && phoSCEta[i] < 1.479) bin = 1;
  else if (phoSCEta[i] >= 1.479 && phoSCEta[i] < 2.0) bin = 2;
  else if (phoSCEta[i] >= 2.0 && phoSCEta[i] < 2.2) bin = 3;
  else if (phoSCEta[i] >= 2.2 && phoSCEta[i] < 2.3) bin = 4;
  else if (phoSCEta[i] >= 2.3 && phoSCEta[i] < 2.4) bin = 5;
  else bin = 6; // phoSCEta[i] >= 2.4
  double corr = phoPFPhoIso[i] - phoEA[bin] * rho;

  //double cut_phoIso[] = {0., 0.}; 
  ...
  cut_phoIso[iEB]= TMath::Max(corr, 0.);
...
float PhotonSSMVA(TreeReader &data, Int_t i, TGraph *tgr[20]) {
  ...
  if (!tmvaReader[iBE]) {
    ...
    // read weight files
    //if (iBE == 0) tmvaReader[0]->BookMVA("BDT", "TMVA/spring16/TMVAnalysis_BDT_EB.weights.xml");
    //else tmvaReader[1]->BookMVA("BDT", "TMVA/spring16/TMVAnalysis_BDT_EE.weights.xml");
    //summer16
    //if (iBE == 0) tmvaReader[0]->BookMVA("BDT", "TMVA/summer16/TMVAnalysis_BDT_EB_v1.weights.xml");
    //else tmvaReader[1]->BookMVA("BDT", "TMVA/summer16/TMVAnalysis_BDT_EE_v1.weights.xml");

    //if (iBE == 0) tmvaReader[0]->BookMVA("BDT", "TMVA/summer16/420/TMVAnalysis_BDT_EB.weights.xml");
    //else tmvaReader[1]->BookMVA("BDT", "TMVA/summer16/420/TMVAnalysis_BDT_EE.weights.xml");

    if (iBE == 0) tmvaReader[0]->BookMVA("BDT", "TMVA/summer16/upto6000/TMVAnalysis_BDT_EB.weights.xml");
    else tmvaReader[1]->BookMVA("BDT", "TMVA/summer16/upto6000/TMVAnalysis_BDT_EE.weights.xml");
...
bool pho_matchfsr (TreeReader &data, int ipho) {
  ...
  for (Int_t i=0; i<nMC; ++i) {
    if (abs(mcPID[i]) != 13 && abs(mcPID[i]) != 15) continue;
    //if ( ((mcStatusFlag[i] >> 0) & 1) == 0 && ((mcStatusFlag[i] >> 1) & 1) == 0 ) continue; 
