...
//float PhotonSSMVA(Int_t i, TGraph *tgr[20]) {
float PhotonSSMVA(TreeReader &data, Int_t i, TGraph *tgr[20]) {
  ...
  if (!tmvaReader[iBE]) {
    ...
    if (iBE == 0) tmvaReader[0]->BookMVA("BDT", "../ana/TMVA/summer16/upto6000/TMVAnalysis_BDT_EB.weights.xml");
    else tmvaReader[1]->BookMVA("BDT", "../ana/TMVA/summer16/upto6000/TMVAnalysis_BDT_EE.weights.xml");
  }
...
bool phoMatch_ele (TreeReader &data, int iPho) {
...
bool phoMatch_jet (TreeReader &data, int iPho) {
  ...
  for(int imc=0; imc<nMC; imc++){
    if (abs(mcPID[imc])!=22) continue;
    if (abs(mcMomPID[imc])<25 &&  abs(mcMomPID[imc])==999) continue;
    if ( deltaR(mcEta[imc],mcPhi[imc],phoEta[iPho],phoPhi[iPho]) < 0.1 ) {
      genmatch = true;
      break;
    }
