...
void anaTnP_v2(TString inputfile = "HiForest.root", TString outputfile = "XZg.root", bool mc = true)
{
  ...
  //outputfile += "_TnP_Zee_BDT_Upto6000_Corr_EtaW_from_Sieie.root";
  outputfile += "_TnP_Zee_BDT_Upto6000_5VarCorr.root";
  ...
  //TFile* fss_rw = TFile::Open("external/transformation_April18_rebin_PhoPresel_v2.root");
  //TFile* fss_rw = TFile::Open("external/transformation_April27_rebin_LooseID_Zmcut.root");
  TFile* fss_rw = TFile::Open("external/transformation_pho_presel_BDTUpto6000.root", "read");
  //TFile* fss_rw = TFile::Open("external/transformation_pho_presel_BDTUpto6000_SieieForEtaW.root", "read");
  ...
  for ( Long64_t ev = 0; ev < TotalEntries; ev++) {   
	...
	if (ProbePt[i] > maxpt) {
      ...
      if ( data.HasMC() ) {
        //reco sf
	int binxrec, binyrec;
        binyrec = heleRecoSF->GetYaxis()->FindBin(elePt[*itag]);
        binxrec = heleRecoSF->GetXaxis()->FindBin(eleSCEta[*itag]);
        eleRecoSF = heleRecoSF->GetBinContent(binxrec, binyrec);
        if (elePt[*itag] < 25.) eleRecoSF = heleRecoSF->GetBinContent(binxrec, 1);
        int NBinsYRec = heleRecoSF->GetNbinsY();
	float maxPt = heleRecoSF->GetYaxis()->GetBinUpEdge(NBinsYRec);
	if ( elePt[*itag] > maxPt) eleRecoSF = heleRecoSF->GetBinContent(binxrec, NBinsYRec);

        //ID SFs
        int binx, biny;
        biny = heleSF->GetYaxis()->FindBin(elePt[*itag]);
        binx = heleSF->GetXaxis()->FindBin(eleSCEta[*itag]);
	eleSF = heleSF->GetBinContent(binx, biny);
        int NBinsY = heleSF->GetNbinsY();
        maxPt = heleSF->GetYaxis()->GetBinUpEdge(NBinsY);
        if ( elePt[*itag] > maxPt) eleSF = heleSF->GetBinContent(binx, NBinsY);
      }
      ...
      */
      ...
      Probe_ssmva_rw = PhotonSSMVA(data, iProbe, tgr);
      Probe_ssmva = PhotonSSMVA(data, iProbe);
      diff_mva = Probe_ssmva - Probe_ssmva_rw;
