...
bool pass_ichep (int iEle) {
  ...
  if (fabs(eleSCEta[iEle]) < 1.4442) {
    if (eleSigmaIEtaIEta_Full5x5[iEle] > 0.0103) pass = false;
    if (fabs(eledEtaAtVtx[iEle]) > 0.0105) pass = false;
    if (fabs(eledPhiAtVtx[iEle]) > 0.115) pass = false;
    if (eleHoverE[iEle] > 0.104) pass = false;
    if (fabs(eleEoverPInv[iEle]) > 0.102) pass = false;
    if (fabs(eleD0[iEle]) > 0.0261) pass = false;
    if (fabs(eleDz[iEle]) > 0.41) pass = false;
    if (eleMissHits[iEle] > 2) pass = false;
    if (eleConvVeto[iEle] != 1) pass = false;
  }
  else {
    if (eleSigmaIEtaIEta_Full5x5[iEle] >0.0301) pass = false;
    if (fabs(eledEtaAtVtx[iEle]) > 0.00814) pass = false;
    if (fabs(eledPhiAtVtx[iEle]) > 0.182) pass = false;
    if (eleHoverE[iEle] > 0.0897) pass = false;
    if (fabs(eleEoverPInv[iEle]) > 0.126) pass = false;
    if (fabs(eleD0[iEle]) > 0.118) pass = false;
    if (fabs(eleDz[iEle]) > 0.822) pass = false;
    if (eleMissHits[iEle] > 1) pass = false;
    if (eleConvVeto[iEle] != 1) pass = false;
  }
...
void select_electron13(vector<int> nele, vector<int> acc_tight, vector<int> acc_medium, vector<int> acc_loose, vector<int> acc_heepId, vector<int> acc_ichep){
  ...
  for (Int_t i=0 ; i<nEle ; ++i){
    ...
    nele       .push_back(i);
    acc_tight  .push_back(pass_cutbased_80X(i, 2));
    acc_medium .push_back(pass_cutbased_80X(i, 1));
    acc_loose  .push_back(pass_cutbased_80X(i, 0));
    acc_heepId .push_back(pass_HEEPID(i));
    acc_ichep  .push_back(pass_ichep(i));

    //if ( pass_cutbased_80X(i, 2)) acc_tight.push_back(i);
    //if ( pass_cutbased_80X(i, 1)) acc_medium.push_back(i);
    //if ( pass_cutbased_80X(i, 0)) acc_loose.push_back(i);
    //if ( pass_HEEPID(i)) acc_heepId.push_back(i);
...
Bool_t Electron_SMZg(int i) {
  ...
  if (elePt[i] < 15.) return false;
  if ((elePt[i] <= 10.) && (eleIDMVAHZZ[i] < lowcut_mva_HZZ[cat])) return false;
  if (fabs(eleSCEta[i]) > 2.5) return false;
  if (fabs(eleD0[i]) > cut_d0[iEB]) return false;
  if (fabs(eleDz[i]) > cut_dz[iEB]) return false;
  if (eleSIP[i] >= 4.)              return false;


  if (elePt[i] >= 10. && eleIDMVAHZZ[i] < cut_mva_HZZ[cat])  return false;
  if (((elePFChIso[i] + std::max (0.0f, elePFNeuIso[i] + elePFPhoIso[i] - rho * cut_effA[eta_bit])) / elePt[i]) >= 0.35) return false;
